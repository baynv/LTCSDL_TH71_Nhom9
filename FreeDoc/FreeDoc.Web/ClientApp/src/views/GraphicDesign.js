...
export default class GraphicDesign extends Component {
    ...
    render() {
        ...
        let elmPosts = posts.map((post, index) => {
            return (
                <Post key={index} id={post.id} thumbnail={post.thumbnail} title={post.title} link={`${url}/${post.link}`} uploadTime={post.upload_time} comments={post.comments} />
            );
        });
        ...
        return (
            <Switch>
                ...
                <Route path='/graphic-design/:link' component={Document}/>
            </Switch>
